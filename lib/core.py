...
class core:
		...
		for form in forms:
			...
			if form["method"].lower().strip() == "post":
				...
				for key in form.find_all(["input","textarea"]):
					v = key.get("value")
					value = v if v else str()
					try:
						if key.get("type") == "submit":
							Log.info("Form key name: "+G+key["name"]+N+" value: "+G+"<Submit Confirm>")
							keys.update({key["name"]:value})
						if key.get("type") == "hidden":
							Log.info(f"Hidden input: name={G}\"{key['name']}\"{N}, value={G}\"{value}\"")
							keys.update({key["name"]:value})
				...
				for key in form.find_all(["input","textarea"]):
					...
					try:
						if key.get("type") == "submit":
							Log.info("Form key name: "+G+key["name"]+N+" value: "+G+"<Submit Confirm>")
							keys.update({key["name"]:value})
						elif key.get("type") == "hidden":
							Log.info(f"Hidden input: name={G}\"{key['name']}\"{N}, value={G}\"{key['value']}\"")
							keys.update({key["name"]:value})
	...
	def get_method(self):
		...
		for a in links:
			url=a["href"]
			if url.lower().startswith("http://") is False or url.lower().startswith("https://") is False or url.lower().startswith("mailto:") is False:
	...
	@classmethod
	def main(self,url,proxy,headers: dict,payload,cookie,method=2, random_agent=False):
		...
		if random_agent:
			...
			Log.info(f"Random User-Agent: {W}{ua}")
